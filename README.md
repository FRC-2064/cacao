...
Without this the app runs in local mode — real and fully usable, but the data
lives in your browser only and starts empty. To point it at the shared
deployment:
...
A fresh deployment starts empty. Open **Admin** and use **Import 2064 data** to
push `src/lib/data/teamData.ts` into Convex; the panel at the top of that page
shows which mode you are in. The button replaces everything in the deployment,
so it is a bootstrap tool, not a routine one.

### The 2064 dataset

`src/lib/data/teamData.ts` is generated, not hand-written. It comes from the
team's two Google Sheets — the grants workbook and the 2024-2026 budget — whose
CSV exports are committed under `scripts/import/data/` alongside a snapshot of
the Hack Club Bank transaction feed. To refresh it:

```bash
# re-export the sheets into scripts/import/data/, then
node scripts/import/generate.ts
```

The mapping rules live in `scripts/import/mapping.ts` and are covered by
`scripts/import/mapping.test.ts`; `scripts/import/dataset.test.ts` then checks
the generated output as a whole.

### The test deployment

The Convex project has two deployments:
...
`.env.local` points at the test one, so spinning up locally can never reach the
real site. To work against a current copy of real data:

```bash
npm run sync:test-data
```

That snapshots production and replaces the test deployment's data with it. It
is read-only against production, and re-running it is always safe — the test
deployment exists to be overwritten. Run it whenever the copy has drifted or
you have made a mess of it.

For a read-only spot check against production itself, override the URL for that
one command rather than editing `.env.local`:

```bash
PUBLIC_CONVEX_URL=https://festive-lion-592.convex.cloud npm run dev
```

**What the dataset deliberately leaves out.** The app fetches every Hack Club
Bank transaction live from the HCB API on load, so the budget sheet's ~97 Hack
Club donation rows, its 24 fiscal-sponsorship fee rows, and the 48 purchases the
bank feed already reports are *not* imported. Importing them too would count the
same money twice. What is imported is what HCB cannot know: grants, sponsorship
cheques, fundraisers banked elsewhere, school-PO and reimbursed purchases, and
in-kind gifts.
...
Production deploys are driven by **version tags**, not by pushes. Nothing
reaches `finance.2064.team` until a tag is pushed:
...
`.github/workflows/release.yml` picks the tag up, runs the tests and the
typecheck, and only then asks Vercel for a production deploy. A failing test is
a failed release rather than a broken site. The tag pattern accepts `0.1.0` and
`v0.1.0` alike.

The workflow needs three repository secrets, all already set:
`VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`. It deliberately does not
build anything itself — the build happens on Vercel, which is where
`CONVEX_DEPLOY_KEY` lives.

Running the workflow manually (**Actions → Release → Run workflow**) runs the
test gate and stops without deploying.
...
1. Generate a production deploy key and add it to Vercel as
   `CONVEX_DEPLOY_KEY` (Sensitive). Either from the Convex dashboard under
   **Settings → Deploy Keys**, or from the CLI:

   ```bash
   npx convex deployment token create --prod vercel-cacao
   ```

2. The **Build Command** is `npm run build:vercel`, set in `vercel.json` so it
   lives in git rather than only in the dashboard. The script only runs
   `convex deploy` when `CONVEX_DEPLOY_KEY` is present, so preview builds --
   which have no key -- fall back to a plain `npm run build` instead of
   failing.

That script runs `convex deploy --cmd 'npm run build'`, which pushes `convex/`
to the production deployment and injects that deployment's URL as
`PUBLIC_CONVEX_URL` for the SvelteKit build.

`PUBLIC_CONVEX_URL` **must also be set as a plain Vercel environment variable**
pointing at the same deployment. The build-time injection alone is not enough:
`src/lib/convex/client.ts` reads through `$env/dynamic/public`, which resolves
from the running function's environment on each request, not from a value baked
in at build time. Set it as a *Config* variable, not a *Sensitive* one —
Vercel's Sensitive variables do not reach the SvelteKit runtime, and the app
silently falls back to local mode with an empty URL when it cannot read one.
