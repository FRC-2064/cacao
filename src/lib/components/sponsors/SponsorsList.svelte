<script lang="ts">
  ...
  /**
   * The contact a sponsor points at, or undefined.
   *
   * Undefined covers three cases that are not distinguishable from here and do
   * not need to be: no contact attached, a caller who is not signed in (both
   * `primaryContactId` and `api.contacts.list` are gated), and a contact this
   * caller cannot see. All three render as no contact line.
   */
  function contactFor(primaryContactId?: string) {
    if (!primaryContactId) return undefined;
    return cacao.contacts.find((c) => c._id === primaryContactId);
  }
...
<div class="space-y-4">
  ...
  <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
    {#each filteredSponsors as sponsor (sponsor._id)}
      ...
      {@const primaryContact = contactFor(sponsor.primaryContactId)}
      <article
      ...
      >
        ...
        <!-- Resolved through the gated `contacts` list rather than read off
             the sponsor. `primaryContactId` reaches signed-in members only and
             the contacts themselves are gated too, so a stranger sees nothing
             here -- which is the point: the sponsor row is public and this
             adult's details are not. -->
        {#if primaryContact}
          <p
          ...
          >
            ...
            <span class="truncate">{primaryContact.name}</span>
          </p>
        {/if}
        ...
        <div class="flex items-center gap-1.5">
          {#each [2024, 2025, 2026] as yr}
            ...
            <span
              class="chip chip-sm type-num flex-1 justify-center"
