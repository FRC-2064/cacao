<script lang="ts">
  ...
  let email = $state('');
  let phone = $state('');
  // The `sponsors` row id. A contact carries `sponsorId`; `sponsorName` was a
  // stored copy of the organisation's name and is gone.
  let sponsorId = $state('');
  ...
  const orgOptions = $derived([
    ...
    ...cacao.sponsors.map((sp) => ({ value: sp._id, label: sp.name }))
  ]);
</script>
...
>
  <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-4">
    ...
    <div class="grid gap-3 sm:grid-cols-2">
      <M3Select label="Linked organization" bind:value={sponsorId} options={orgOptions} />
