    ...
    if (cacao.searchQuery.trim()) {
      ...
      list = list.filter(
        (g) =>
          ...
          g.assigneeName?.toLowerCase().includes(q)
      );
    }
...
<PageHeader
  title="Grants List"
  stat={`Showing ${sortedGrants.length} of ${cacao.grants.length} grants`}
>
  {#snippet actions()}
    ...
    <button type="button" class="btn btn-filled" onclick={onaddgrant}>
      <Plus size={18} />
      <span>New grant</span>
    </button>
  {/snippet}
...
<div class="space-y-3 md:hidden">
  {#each sortedGrants as grant (grant._id)}
    ...
    <div
      ...
      onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && onselectgrant(grant)}
    >
      <div class="mb-1 flex items-start justify-between gap-2">
        <span class="type-label-sm truncate" style="color: var(--color-on-surface-variant)">
          {grant.funder}
        </span>
        <span class={`chip chip-sm shrink-0 ${TONE_CHIP[meta.tone]}`}>{meta.label}</span>
      </div>
